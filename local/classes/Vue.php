...
class Vue
{
    ...
    public static function includeComponent($componentName, array $addFiles = [])
    {
        if (self::$init !== true) {
            ...
            if (defined('RICHE_ADD_JS') && RICHE_ADD_JS === true) {
                Asset::getInstance()->addJs('https://unpkg.com/vuex@3.5.1/dist/vuex' . (!defined('RICHE_DEV') ? '.min' : '') . '.js');
                Asset::getInstance()->addJs('https://unpkg.com/vue@2.6.11/dist/vue' . (!defined('RICHE_DEV') ? '.min' : '') . '.js');
            }

            \AddEventHandler('main', 'OnEndBufferContent', ['\Local\Vue', 'insertComponents']);
        }
        ...
        foreach ((array)$componentName as $name) {
            ...
            if (!defined('RICHE_DEV') && file_exists($rootPath . '/' . $name . '/script.min.js')) {
            ...
            if (!defined('RICHE_DEV') && file_exists($rootPath . '/' . $name . '/style.min.css')) {
    ...
    public static function insertComponents(&$content)
    {
        ...
        if (
            ...
            $content = System::minifyContent($content, RICHE_VUE_MINIFY);
        }
    }
    ...
    {
        if (defined('RICHE_VUE_PATH')) {
            return '/' . trim(RICHE_VUE_PATH, '/');
        }
